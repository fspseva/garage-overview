...
const COLLECTION_IDS = Object.fromEntries(Object.entries(COLLECTION_NAMES).map(([id, name]) => [name.toLowerCase(), id]));
// Helper functions for collection resolution
function resolveCollectionId(collectionIdentifier) {
...
function formatCollectionInfo(collectionId) {
...
async function makeApiRequest(network, endpoint, environment = "production") {
    ...
    try {
        ...
        switch (name) {
            ...
            case "garage_get_collections": {
                const { network, ...queryParams } = args;
                ...
                const result = await makeApiRequest(network, endpoint, environment);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            }
            case "garage_get_featured_collections": {
                const { network, limit } = args;
            ...
            case "garage_get_collection_details": {
                const { network, collectionId, fromDate } = args;
            ...
            case "garage_get_collection_nfts": {
                const { network, collectionId, ...queryParams } = args;
                const resolvedCollectionId = resolveCollectionId(collectionId);
                const filteredParams = {};
                Object.entries(queryParams).forEach(([key, value]) => {
                    if (value !== undefined && value !== "production" && key !== "environment") {
            ...
            case "garage_get_nft_details": {
                const { network, nftId } = args;
                const endpoint = `nfts/${nftId}`;
                const result = await makeApiRequest(network, endpoint, environment);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            }
            case "garage_get_nft_activities": {
                const { network, nftId, ...queryParams } = args;
                const filteredParams = {};
                Object.entries(queryParams).forEach(([key, value]) => {
                    if (value !== undefined && value !== "production" && key !== "environment") {
                        filteredParams[key] = String(value);
            ...
            case "garage_get_order_details": {
                const { network, orderId } = args;
                const endpoint = `orders/${orderId}`;
                const result = await makeApiRequest(network, endpoint, environment);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            }
            case "garage_get_user_orders": {
                const { network, sellerAddress, ...queryParams } = args;
                const filteredParams = {};
                Object.entries(queryParams).forEach(([key, value]) => {
                    if (value !== undefined && value !== "production" && key !== "environment") {
                        filteredParams[key] = String(value);
